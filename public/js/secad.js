...
async function initSecad() {
  try {
    ...
    estado.caps = data.caps || { is_master: false, pode_validar: true, pode_comunicados: false, pode_concessionarios: false };
...
function bootApp() {
  const caps = estado.caps || { is_master: false, pode_validar: true, pode_comunicados: false, pode_concessionarios: false };
  const podeValidar = caps.is_master || caps.pode_validar;
  const podeComunicados = caps.is_master || caps.pode_comunicados;
  const podeConcessionarios = caps.is_master || caps.pode_concessionarios;

  // Sidebar: mostra Concessionários Renovação, Comunicados e/ou Cadastro conforme capacidade.
  ...
  document.getElementById('nav-concessionarios-cadastro').style.display = podeConcessionarios ? '' : 'none';
  ...
  if (podeConcessionarios) bootConcessionariosCadastro();
  ...
  mostrarView(podeValidar ? 'validacao' : (podeComunicados ? 'comunicados' : 'concessionarios-cadastro'));
}

const VIEWS_SECAD = {
  validacao: { nav: 'nav-validacao', view: 'view-validacao', titulo: 'Concessionários Renovação', sub: 'Conferência do Setor de Cadastro · CEASAMINAS' },
  comunicados: { nav: 'nav-comunicados', view: 'view-comunicados', titulo: 'Comunicados', sub: 'Setor de Cadastro · Departamento de Operações · CEASAMINAS' },
  'concessionarios-cadastro': { nav: 'nav-concessionarios-cadastro', view: 'view-concessionarios-cadastro', titulo: 'Concessionários Cadastro', sub: 'Cadastro sincronizado do ERP TOTVS (CORPORE) · CEASAMINAS' },
};

// Alterna entre as 3 seções (sidebar): Concessionários Renovação × Comunicados × Concessionários Cadastro.
function mostrarView(view) {
  Object.entries(VIEWS_SECAD).forEach(([key, v]) => {
    document.getElementById(v.view).style.display = key === view ? '' : 'none';
    document.getElementById(v.nav).classList.toggle('active', key === view);
  });
  const atual = VIEWS_SECAD[view];
  document.getElementById('dp-page-title').textContent = atual.titulo;
  document.getElementById('dp-page-subtitle').textContent = atual.sub;
  if (view === 'comunicados') carregarComunicados();
  if (view === 'concessionarios-cadastro') carregarDashboardConcessionarios();
}
...
// ── Concessionários Cadastro (dado sincronizado do CeasaConecta-Gateway) ────────
...
function bootConcessionariosCadastro() {
...
function trocarCcTab(tab) {
  ...
  if (tab === 'pesquisar' && !estado.ccListaCarregada) { estado.ccListaCarregada = true; carregarListaConcessionarios(); }
}
...
async function carregarDashboardConcessionarios() {
  let d;
  try { d = await (await fetch('/api/secad/concessionarios/dashboard')).json(); } catch { return; }
...
async function carregarListaConcessionarios() {
  ...
  let linhas;
  try { linhas = await (await fetch('/api/secad/concessionarios?' + params.toString())).json(); } catch { linhas = []; }
...
async function abrirDetalheConcessionario(codigo) {
  let d;
  try {
    const res = await fetch(`/api/secad/concessionarios/${codigo}`);
...
async function gerarRelatorioConcessionariosPdf() {
  ...
  try {
    const res = await fetch('/api/secad/concessionarios/relatorio/pdf', {
