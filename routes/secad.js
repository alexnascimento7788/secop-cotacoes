...
const { gerarPdfComunicados, gerarPdfConcessionarios } = require('../secad-pdf');
...
function secadCaps(req) {
  if (req.user.username === 'master') return { is_master: true, is_consulta: false, pode_validar: true, pode_comunicados: true, pode_concessionarios: true };
  if (req.user.role === 'consulta') return { is_master: false, is_consulta: true, pode_validar: true, pode_comunicados: true, pode_concessionarios: true };
  if (!req.user.perfil_id) return { is_master: false, is_consulta: false, pode_validar: false, pode_comunicados: false, pode_concessionarios: false };
  const rows = db.prepare(`
    SELECT r.slug, pr.ver
    FROM perfil_rotinas pr JOIN rotinas r ON r.id = pr.rotina_id
    WHERE pr.perfil_id = ? AND r.slug IN ('validacao', 'comunicados', 'concessionarios-cadastro')
  `).all(req.user.perfil_id);
  const mapa = Object.fromEntries(rows.map(r => [r.slug, !!r.ver]));
  return { is_master: false, is_consulta: false, pode_validar: !!mapa.validacao, pode_comunicados: !!mapa.comunicados, pode_concessionarios: !!mapa['concessionarios-cadastro'] };
}
...
// ── SECAD: Concessionários Cadastro (nome/CNPJ/endereço/contrato) ────────────
// Dado sincronizado automaticamente do CeasaConecta-Gateway (ver
...
// Agrupa as linhas de contrato por concessionário (codigo) — usado no
// dashboard, na pesquisa e no relatório, pra nunca divergir o que conta
// como "ativo" entre as telas. `principal` = a linha ativa (ou a 1ª, se
// nenhuma ativa) — é o que aparece como "o" registro daquele concessionário
// nas telas que mostram 1 linha por empresa.
...
router.get('/api/secad/concessionarios/dashboard', secad, requireRotina('concessionarios-cadastro', 'ver'), (req, res) => {
...
// e pelo Relatório. Diferente de concessionariosAgrupados() (usado só no
// Dashboard): um concessionário pode ter mais de 1 contrato ATIVO ao mesmo
// tempo (achado revisando com o Alex) — "achatar" pra 1 linha por empresa
...
router.get('/api/secad/concessionarios', secad, requireRotina('concessionarios-cadastro', 'ver'), (req, res) => {
...
router.get('/api/secad/concessionarios/:codigo', secad, requireRotina('concessionarios-cadastro', 'ver'), (req, res) => {
...
router.post('/api/secad/concessionarios/relatorio/pdf', secad, requireRotina('concessionarios-cadastro', 'ver'), async (req, res) => {
  ...
  try {
    ...
    console.error('[secad] erro ao gerar PDF de concessionarios:', e);
